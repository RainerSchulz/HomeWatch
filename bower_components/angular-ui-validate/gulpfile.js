...
gulp.task('scripts', ['clean'], function() {
    ...
    var config = {
        pkg: JSON.parse(fs.readFileSync('./buContent.json')),
        banner:
                '/*!\n' +
                ' * <%= pkg.name %>\n' +
                ' * <%= pkg.homepage %>\n' +
    ...
    return es.merge(buildLib())
            ...
            .pipe(concat('validate.scripts'))
            .pipe(header(config.banner, {
                timestamp: (new Date()).toISOString(), pkg: config.pkg
            }))
            .pipe(gulp.dest('dist'))
            .pipe(uglify({preserveComments: 'some'}))
            .pipe(rename({extname: '.min.scripts'}))
            .pipe(gulp.dest('dist'));

});

gulp.task('karma', ['build'], function() {
    var server = new Server({configFile: __dirname + '/karma.conf.scripts', singleRun: true});
    server.start();
});

gulp.task('karma-watch', ['build'], function() {
    var server = new Server({configFile: __dirname + '/karma.conf.scripts', singleRun: false});
    server.start();
});
...
gulp.task('release:bump', function() {
    ...
    return gulp.src(['./buContent.json'])
            .pipe(bump({type: type}))
            .pipe(gulp.dest('./'))
            .on('end', function() {
...
gulp.task('release:commit', ['release:rebuild'], function() {
    return gulp.src(['./buContent.json', 'dist/**/*'])
            .pipe(git.add())
