var widget_chart = {
	...
	getStyleRuleValue: function(res, style, selector) { // helper function for getting style values
		...
		//console.log(selector, style, $(selector.replace(' ','.')).css(style), window_hm.getComputedStyle(elem,null).getPropertyValue(style));
		//ret = window_hm.getComputedStyle(elem,null).getPropertyValue(style).indexOf('#');
	...
	refresh: function (elem,type,swoffset) { // main function for generation of all HTML code and dynamics for graph called whenever thigs change (e.g. data update, shift, scale, ...)
		...
		if (xticks == -1) {
			...
			xticks = (data.basewidth>400) ? mdiff/12.0 : (data.basewidth>200) ? mdiff/6 : mdiff/3;	// set the number of ticks to 12 or 6 if window_hm is not so wide
		}
		...
		deviceElements.each(function(index) {
			...
			if ( $(this).data('get')==par && isLogdevice) {
				if ($(this).parent().is(':visible')) {
					waitForInitialization($(this),$(this).data('instance'),'start'); // need to be sure that window_hm is initialized (e.g. to get right width/height)
				} else {
