...
var widget_dimmer = $.extend({}, widget_famultibutton, {
    widgetname : 'dimmer_gira',
    init: function () {
     var base = this;
     this.elements = $('div[data-type="'+this.widgetname+'"]');
     this.elements.each(function(index) {
         var elem = $(this);
         elem.initData('off-color'           , getStyle('.dimmer_gira.off','color') || '#2A2A2A');
         elem.initData('off-background-color', getStyle('.dimmer_gira.off','background-color')   || '#505050');
         elem.initData('on-color'            , getStyle('.dimmer_gira.on','color')               || '#2A2A2A');
         elem.initData('on-background-color' , getClassColor(elem) || getStyle('.dimmer_gira.on','background-color')    || '#aa6900');
         ...
         elem.initData('min'                 , 0);
         elem.initData('mode','dimmer_gira');
