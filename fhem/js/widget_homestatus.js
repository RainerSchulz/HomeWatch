...
var widget_homestatus = $.extend({}, widget_widget, {
    ...
    if (sector==1){
        ...
        //cavans font
        var cfont=8*window.devicePixelRatio +"px sans-serif";
        var cfafont=22*window.devicePixelRatio +"px FontAwesome";
        var coffset=5*window.devicePixelRatio;
        var sizem=12*window.devicePixelRatio +"px FontAwesome";
        
        c.fillStyle = (sector==1)?this.o.minColor:this.o.maxColor;
        c.font = cfont;
        c.fillText(this.o.texts[0], this.xy-10, this.xy+9+coffset);
        c.font = cfafont;
        ...
        if (this.o.texts.length > 4){
            ...
            c.font = cfont;
            c.fillText(this.o.texts[2], this.xy-23, this.xy+this.radius*0.82+coffset);
        }
    ...
    init_attr : function(elem) {
       /* var defaultAlias =  new Array('Home','Night','Away','Holiday','Retire');*/
        ...
        var defaultAlias =  new Array('Test','Kino','leaving home','Comin home');
        var defaultIcons =  new Array('fa-home','fa-bed','fa-car','fa-suitcase');
        var defaultStates = new Array('1','2','3','4');

        if ( elem.data('version')  && elem.data('version') != ''){
            defaultStates = new Array('Test','Kino','leaving home','Coming home');
        }
    ...
    init_ui : function(elem) {
        ...
        knob_elem.knob({
            ...
            'bgColor': elem.data('bgcolor') || '#FFFFFF',
            'fgColor': elem.data('fgcolor') || '#FFFFFF',
            'tkColor': elem.data('tkcolor') || '#FFFFFF',
            'minColor': '#2A2A2A',
            'maxColor': '#FFFFFF',
            'thickness': 0.4,
            'lastvalue':0,
